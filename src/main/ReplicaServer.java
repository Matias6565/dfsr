...
public class ReplicaServer implements ReplicaServerClientInterface {
	...
	public boolean commit(long txnID, long numOfMsgs) throws MessageNotFoundException,
		...
		System.out.println("REPLICA : NUM OF MESSAGES : " + lockData.noOfMessages.get());
	...
	@Override
	public void removeReplicas(String fileName, Address[] replicas)
			throws NotBoundException, IOException {		
		//debug
		/*File file1 = new File("/home/alisson/workspace/DFS-yra/bin/ServersData/Replica4/out3.txt");
		FileWriter fw = new FileWriter(file1);
		PrintWriter pw = new PrintWriter(fw);
		pw.println("\n"+replicaLocations.size());
		pw.println("\n"+directory + METADATA);
		pw.println("\n"+replicas[0].objectName);
		pw.close();*/
		
		//apagar metadata na replica primaria
		replicaLocations.remove(fileName);
		if(replicaLocations.size() >= 1) {
			MethodsUtility.writeMetaData(directory + METADATA, replicaLocations);
			MethodsUtility.removeFromDisk(fileName, directory, replicas); //apagar os arquivos
		}
		else {
			MethodsUtility.removeMetaData(directory + METADATA);
			MethodsUtility.removeFromDisk(fileName, directory, replicas); //apagar os arquivos
		}
